"""Competition evidence pack — single-command multi-round adversarial summary.
...
def _plot_convergence_curve(run_dir: Path, convergence: dict[str, Any]) -> Path:
    ...
    ax1.set_title("Adversarial Self-Evolution Convergence")
...
def _plot_damage_radar(run_dir: Path, convergence: dict[str, Any]) -> Path:
    ...
    ax.set_title("Damage Radar (before vs after hardening)", y=1.08)
...
def _write_datacard(run_dir: Path, convergence: dict[str, Any], metrics: dict[str, Any]) -> Path:
    ...
    lines.append("")
    lines.append("```bash")
    lines.append("python run.py --evidence-pack --offline   # 离线确定性复现")
    lines.append("```")
    lines.append("")
    lines.append("## 边界")
    lines.append("")
